...
(defcustom org-recoll-command-invocation "docker exec recoll_python python3 /root/scripts/recollpy.py "
...
(defcustom org-recoll-index-invocation "docker exec recoll_python recollindex"
...
If PAGING is t this indicates that the function is being called to page through results."
  ...
  (insert (shell-command-to-string (concat org-recoll-command-invocation " '" (org-recoll-sanitize-single-quote squery) "'")))
